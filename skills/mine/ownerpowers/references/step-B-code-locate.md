# step-B · 代码定位套路

## 需求线定位

自顶向下沿调用链定位改动点：

```
Controller（入口/参数/返回）
  → Service（业务逻辑/事务边界）
    → Mapper / XML / SQL（数据访问）
```

先找接口对应的 Controller 方法，再顺着 Service 调用链找到要改的逻辑点与数据访问点。

## 排查线定位

- **有报错堆栈**：从堆栈最顶层业务类逐层下钻，定位抛异常的真实位置（区分框架栈与业务栈）。
- **无堆栈、按现象**：从现象涉及的接口/功能入口反查，结合日志关键字搜索。

## 与 spec-architect 的关系

若本任务已委托 spec-architect，spec 内已含勘察定位结论，
step-B **退化为"按 spec 列出的文件/改动点核对"**，不重复勘察。

## 工具

- 优先用项目内搜索（Grep/Glob）按类名、方法名、SQL 片段、报错关键字定位。
- 大范围、不确定命中位置时，可派 Explore/general-purpose 子代理并行检索。

## 🔍 subagent 探针（定位时判）

- 定位发现涉及 **2+ 独立模块 / 文件组**（互不依赖）→ 按 [`../policies.md`](../policies.md) **并行 fan-out**。
- 需**大量探查 / 通读、但只取一个结论性结果**（如"查清这条链路在哪断"）→ 按 subagent.md **探查卸载**派单个 subagent，主 session 只接结论（省主上下文，任何档可用）。
- subagent 一律 `opus`，effort 按复杂度；产出回来走 `../disciplines.md` 核对（看 diff，不轻信自述）。
